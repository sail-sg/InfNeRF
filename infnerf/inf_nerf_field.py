...
from nerfstudio.field_components.encodings import HashEncoding, NeRFEncoding, SHEncoding, Identity
...
class InfNerfField(Field):
    ...
    def __init__(
        ...
        #embedding_appearance: Optional[Embedding] = None,#todo move to root or module
        #num_downscales: Optional[int] = 1
    ) -> None:
        super().__init__()
        ...
        #if embedding_appearance is not None:
        #    self.embedding_appearance = embedding_appearance
        #else:
        ...
        #self.direction_encoding = Identity(3)
    ...
    def get_density2(self, positions) -> Tuple[Tensor, Tensor]:
        """Computes and returns the densities."""
        if self.spatial_distortion is not None:
            positions = self.spatial_distortion(positions)
            positions = (positions + 2.0) / 4.0
    ...
    def get_outputs(
        ...
        h = torch.cat(
            [
                d,
                density_embedding.view(-1, self.geo_feat_dim).sigmoid(),#sigmoid can prevent NaN after long train
                embedded_appearance.view(-1, self.appearance_embedding_dim),
            ],
            dim=-1,
        )
        rgb = self.mlp_head(h).view(*outputs_shape, -1).to(directions)
        # rgb = mlp_head(h).view(*outputs_shape, -1).to(directions) # use only one rgb mlp
        outputs.update({FieldHeadNames.RGB: rgb})

        return outputs
    #for color interlevel loss
    # def get_outputs2(
    #     self, positions: Tensor, directions: Tensor, #[...,3]
    #     camera_indices: Tensor,#[...,1]
    #     density_embedding: Tensor,#[...,15]
    # ) -> Dict[FieldHeadNames, Tensor]:
    #     assert density_embedding is not None
    #     outputs = {}
    #     # if ray_samples.camera_indices is None:
